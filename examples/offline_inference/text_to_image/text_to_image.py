...
def main():
    ...
    if args.pid_enable:
        ...
        if args.pid_gemma_path is not None:
            omni_kwargs["pid_gemma_path"] = args.pid_gemma_path
    ...
    if args.pid_enable:
        print(
            f"  PiD: enabled, scale={args.pid_scale or 'default'}, "
            f"num_steps={args.pid_num_steps or 'default'}, seed={args.pid_seed or 'default'}"
        )
    print(f"{'=' * 60}\n")
